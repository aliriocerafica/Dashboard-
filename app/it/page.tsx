...
import { fetchITData, calculateITStats, ITData, ITDashboardStats } from '../lib/sheets';
...
const GOOGLE_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1OO2MhxCnpLYM3d2IuX7EFqVtGOZ4HvhE2SCgVei5hvs/edit?gid=489896504#gid=489896504';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-1">IT Dashboard</h1>
            <p className="text-sm text-gray-600">Track and manage support tickets and IT operations</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 shadow-lg text-white">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm font-medium opacity-90">Total Tickets</div>
            </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
            ...
            <div className="text-4xl font-bold text-gray-900 mb-1">{stats.resolvedTickets}</div>
            <div className="text-sm text-gray-500">{stats.unresolvedTickets} pending</div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
            ...
            <div className="text-4xl font-bold text-gray-900 mb-1">{stats.avgResolutionTime}</div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
            ...
            <div className="text-4xl font-bold text-gray-900 mb-1">{stats.satisfactionRate}%</div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-4">
          ...
          <div className="xl:col-span-2">
            <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-100">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Troubleshooting Types</h3>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {Object.entries(actualTicketData.reduce((acc, item) => {
                ...
                .map(([type, count]) => (
                  <div key={type} className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold bg-purple-600 text-white px-2 py-0.5 rounded-full">{count}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-100">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-emerald-600">{stats.resolvedTickets}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Unresolved</span>
                  <span className="text-lg font-bold text-red-600">{stats.unresolvedTickets}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Total</span>
                  <span className="text-lg font-bold text-gray-900">{stats.totalTickets}</span>
                </div>
                <div className="pt-3 border-t border-gray-200">
                  <div className="text-xs text-gray-500 mb-1">Satisfaction Rate</div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{actualTicketData.length} Total</p>
            </div>
            <button 
              ...
              <tbody>
                {actualTicketData.slice(0, 10).map((ticket, index) => (
                  <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-2 text-gray-600">{ticket.timestamp.split(',')[0]}</td>
                    <td className="py-3 px-2 text-gray-900 font-medium truncate max-w-xs">{ticket.fullName || 'N/A'}</td>
                    <td className="py-3 px-2 text-gray-600 truncate max-w-xs">{ticket.account}</td>
                    <td className="py-3 px-2 text-gray-600 truncate max-w-xs">{ticket.troubleshootingType.split(',')[0]}</td>
                    ...
                    <td className="py-3 px-2 text-gray-600">{ticket.timeResolved || 'N/A'}</td>
                    <td className="py-3 px-2 text-gray-600">{ticket.employeeRating || 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex min-h-screen items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[90vh] flex flex-col">
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-purple-100 rounded-t-2xl">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">All IT Tickets</h2>
                  <p className="text-sm text-gray-600 mt-1">Complete ticket history with search and filter</p>
                </div>
                <button
              ...
              <div className="p-6 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  {actualTicketData.filter(ticket => 
                    ticket.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ticket.account?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ticket.troubleshootingType?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ticket.status?.toLowerCase().includes(searchTerm.toLowerCase())
                  ).length} of {actualTicketData.length} tickets match your search
                </p>
              </div>
                    ...
                    {actualTicketData
                      ...
                      .map((ticket, index) => (
                        <tr key={index} className="border-b border-gray-100 hover:bg-purple-50/50 transition-colors">
                          <td className="py-3 px-4 text-gray-600">{ticket.timestamp.split(',')[0]}</td>
                          <td className="py-3 px-4 text-gray-900 font-medium">{ticket.fullName || 'N/A'}</td>
                          <td className="py-3 px-4 text-gray-600">{ticket.account}</td>
                          <td className="py-3 px-4 text-gray-600">{ticket.troubleshootingType.split(',')[0]}</td>
                          ...
                          <td className="py-3 px-4 text-gray-600">{ticket.timeResolved || 'N/A'}</td>
                          <td className="py-3 px-4 text-gray-600">{ticket.employeeRating || 'N/A'}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
              ...
              <div className="p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
                <div className="flex justify-between items-center">
                  <p className="text-sm text-gray-600">
                    Total: {actualTicketData.length} tickets | Filtered: {actualTicketData.filter(ticket => 
                      ticket.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                      ticket.account?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                      ticket.troubleshootingType?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                      ticket.status?.toLowerCase().includes(searchTerm.toLowerCase())
